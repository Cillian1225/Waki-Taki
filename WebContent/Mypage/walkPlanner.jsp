...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick:function(eventData) { //일정보기,수정
	            ...
	            $("#view_place_name").val(eventData.event.title);
	            $("#view_start_date").val(start.slice(0,-1));
	            $("#view_end_date").val(end.slice(0,-1));
	            
	            $("#view_contents").val(
	            	'person_id : ' + eventData.event.extendedProps.person_id + ', ' + 	
	...
	/*             let okbtn = $("<button>");
	            okbtn.addClass("btn btn-warning");
	            
	            if(eventData.event.extendedProps.person_id == ${login.id}){
	            	$("#view_modal_footer").append();
	            	<button type="button" class="btn btn-warning" id="view_ok" data-dismiss="modal">확인</button>
                    <button type="button" class="btn btn-warning" id="view_update" data-dismiss="modal">수정</button>
                    <button type="button" class="btn btn-warning" id="view_update" data-dismiss="modal">삭제</button>
	            ...
	            $("#viewModal").modal("show"); 
         		...
         		$("#view_update").on("click",function(){
         			$.ajax({ 
            			url:"update.planner",
            			type:"get",
            			data:{"seq" : seq, "title" : $("#view_place_name").val(), "start" : $("#view_start_date").val(),"end" : $("#view_end_date").val()}
            		}).done(function(resp){
            			if(resp =="1"){
            				alert("일정 수정 완료!");
            			}else{
        ...
        <div class="row m-0">
            ...
            <c:choose>
            	<c:when test="${login==null}">
            		...
            		<div class="col-6 col-lg-4 col-xl-1 p-0 navitext"><a href="${pageContext.request.contextPath}/person_signup.jsp">회원가입</a></div>
            	</c:when>
            	<c:otherwise>
            		<div class="col-6 col-lg-4 col-xl-1 p-0 navitext" id="mypage"><a href="Mypage.mem">마이페이지</a></div>
           			<div class="col-6 col-lg-4 col-xl-1 p-0 navitext"><a href="logoutProc.mem">로그아웃</a></div>
            	</c:otherwise>
            </c:choose>
        </div> 
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group m-0">
                        ...
                        <input type="text" class="form-control" id="view_place_name" name="view_place_name">
                        <label for="taskId" class="col-form-label">시작 날짜</label>
                        <input type="datetime-local" class="form-control" id="view_start_date" name="view_start_date">
                        <label for="taskId" class="col-form-label">종료 날짜</label>
                        <input type="datetime-local" class="form-control" id="view_end_date" name="view_end_date">
                        <label for="taskId" class="col-form-label">강아지 정보</label>
                        <textarea class="form-control" rows="6" name="view_contents" id="view_contents"></textarea>
                    </div>
                </div>
