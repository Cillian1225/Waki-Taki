...
public class MessageController extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(url.contentEquals("/mypage/plannerAdd.message")) { // planner에서 쪽지 보내기
			...
			}else if(url.contentEquals("/mypage/check.message")) {
				int pb_seq = Integer.parseInt(request.getParameter("pb_seq"));
			...
			}else if(url.contentEquals("/mypage/accept.message")) { // 쪽지 수락
				...
				if(m_result>0) { // planner에 일정등록 + 쪽지 삭제
			...
			}else if(url.contentEquals("/mypage/cancel.message")) { // 쪽지 삭제 취소
